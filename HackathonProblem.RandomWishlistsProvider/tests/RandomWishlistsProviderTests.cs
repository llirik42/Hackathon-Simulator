...
public class WishlistsProviderTests : RandomWishlistsProviderTestsFixture
{
    [Theory]
    ...
    public void TestWishlistsSize(List<Employee> teamLeads, List<Employee> juniors)
    {
        ...
        var teamLeadsIds = teamLeads.Select(j => j.Id).ToList();

        var juniorsWishlists = provider.ProvideJuniorsWishlists(juniorsIds, teamLeadsIds);
        var teamLeadsWishlists = provider.ProvideTeamLeadsWishlists(juniorsIds, teamLeadsIds);
    ...
    public void TestWishlistsOwners(List<Employee> teamLeads, List<Employee> juniors)
    {
        ...
        Assert.Equal(teamLeadsWishlists.Select(w => w.EmployeeId).ToHashSet(), teamLeadsIds.ToHashSet());
        Assert.Equal(juniorsWishlists.Select(w => w.EmployeeId).ToHashSet(), juniorsIds.ToHashSet());
    }
    ...
    public void TestWishlistsDesiredEmployees(List<Employee> teamLeads, List<Employee> juniors)
    {
        ...
        foreach (var tw in teamLeadsWishlists) Assert.Equal(tw.DesiredEmployees.ToHashSet(), juniorsIds.ToHashSet());

        foreach (var jw in juniorsWishlists) Assert.Equal(jw.DesiredEmployees.ToHashSet(), teamLeadsIds.ToHashSet());
    }
}
